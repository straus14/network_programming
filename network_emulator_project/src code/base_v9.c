/* base file for network emulation v6*/
/* includes */
#include <stdio.h>
#include <string.h>
...
if <emulation command> and <statistic> are skipped, this program just works are a simple packet forwarder.
...
out of order number of frame to get out of order

Example to emulate packet error with 14% with two random local host , command is,

      sudo ./executable loss 14 192.168.1.10 192.168.1.58
...
//record the time at the beginning
 ...
 struct timeval program_begin, program_finished, before, after;
...
*/
 ...
 if (argc < 5)
 {
	 ...
	 printf("statistic: microseconds (d), percentage number (l & e), number of frames before periodic out of order (o)\n");
	 printf("(with command) example: ./file e 14 <ip1> <ip2>\n");
	 printf("(without command) example: ./file <ip1> <ip2>\n");
	 exit(0);
 }
...
while(1)
{
  ...
  z = select (packet_socket+1,&read_fds, NULL, NULL, &timeout);
    if (z == -1) {
       ...
       else if (!z);// printf("...select timeout\n"); /* timeout is represented by three dots */

    	 /*  gettimeofday(&after , NULL);
    	   if(time_diff(before , after)>100) //exit loop when timer lasts more than 100us to print summary file
    		   break;
    		   */
...
//---------------------> call the emulation function with send function here (pck_size as flag).
   ...
   {
	   pktSentNum++;
	   //gettimeofday(&before , NULL); //reset all timers once we send a pkt
	   //gettimeofday(&after , NULL);
	   ...
	   switch (*argv[1])
             {
                case 'd' :
                	...
                	fprintf(fptr, "%dth delayed packet sent\n", pktSentNum);
                ...
                case 'l' :
                	...
                	if ( rand() <= rand_thrsld){
                		pktLostNum++;
                		fprintf(fptr, "%dth pkt dropped here, pkt seqnum: %s, src IP: %s dest IP: %s\n",
                				pktLostNum, "..", argv[3], argv[4]);
                		      // print source ip, dest ip and packet no
                	}
                	else{
                		sendPkt=TRUE;
				    }
                ...
                case 'e' :
                	...
                	if ( rand () <= rand_thrsld)
                	{
				    	...
				    	fprintf(fptr, "%dth pkt made erroneous here, pkt seqnum: %s, src IP: %s dest IP: %s\n",
				    	                				pktErrNum, "..", argv[3], argv[4]);
                	}
                	sendPkt=TRUE;
                	break;
                ...
                case 'o' :
                	 ...
                	 * the drawback of this method if the number of last packet is a multiple of "delayGap"
                	 * then only that one packet will not be sent
                	 */
                	sendPkt=TRUE;
                	...
                	if(pktSentNum % delayGap == 0){
                		...
                		sendPkt=FALSE;
                		fprintf(fptr, "%dth packet is prepared to be swapped\n", pktSentNum);
                	}
                	...
                	/* method 2
                	 * suppose delayGap = 3
                	 * delay only each multiple of 3 bits by 1 second
                	 * then the resulting packet order will be swapped
                	 */
					//if(pktSentNum % delayGap == 0)
						//sleep(1);
					//end of method 2
   ...
   wrongpacket:/*do nothing*/ ;
	...
	fprintf(fptr, "Network Emulation of overall %d packets transmitted\n", pktSentNum);
	...
	switch (*argv[1])
    {
		...
		case 'e' :
		  ...
		  break;
		case 'o' :
		  fprintf(fptr, "The packet is out of order every %d packet\n", *argv[2]);
		  break;
		default  :
